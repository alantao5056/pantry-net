...
import { PANTRIES } from './pantry-finder/data';
...
function Map({ onPantryHover, hoveredPantryId }: MapProps) {
  ...
  useEffect(() => {
    ...
    mapRef.current = new mapboxgl.Map({
      ...
      center: [-71.06776, 42.35816], // starting position [lng, lat]
    ...
    for (const pantry of PANTRIES) {
      ...
      const marker = new mapboxgl.Marker()
        .setLngLat([pantry.longitude, pantry.latitude])
  ...
  useEffect(() => {
    ...
    Object.entries(markersRef.current).forEach(([id, marker]) => {
      ...
      if (id === hoveredPantryId) {
        ...
        element.style.filter = 'drop-shadow(0 4px 8px rgba(0, 0, 0, 0.25))';
