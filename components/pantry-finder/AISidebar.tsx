    ...
    const sendMessage = async (newMessages: Message[]) => {
        ...
        try {
            ...
            if (responseMessage.error) {
                console.error("Error from /api/chat:", responseMessage);
                return;
            }

            let updatedMessages = [...newMessages, responseMessage];
            ...
            if (responseMessage.tool_calls && responseMessage.tool_calls.length > 0) {
                ...
                // Call the backend again so it can summarize the tool execution
                ...
                {messages.length === 0 && (
                    <div className="flex h-full flex-col items-center justify-center text-center text-gray-500">
                        ...
                        <p className="max-w-[240px] text-sm">
                            Ask me to find pantries, filter results, or explain
                            pantry details!
                        </p>
                    </div>
                )}
                        ...
                        (!messages.length ||
                            ...
                            <div className="flex items-start">
                                <div className="rounded-2xl bg-white px-4 py-3 text-sm shadow-sm">
                                    <Loader2
                                        size={16}
                                        className="animate-spin text-pantry-dark"
                                    />
                                </div>
                            </div>
                        )}
